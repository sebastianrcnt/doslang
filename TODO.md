...
| **GOAL.md P0~P4** | 외부 감사에서 나온 것 전부. stdlib 다섯, 스펙 빈칸, 죽은 C 백엔드, 측정 |
...
| 정수 리터럴 | 문맥이 요구하는 타입, 없으면 `i32`. 범위를 벗어나면 거부 (SPEC §3) |
| store 폭 | 값이 아니라 **자리**가 정한다 |
| unsafe 예산 | `std.mem`/`std.sys` 밖은 0. `run.py` 가 검사하고 늘어나면 실패한다 |
