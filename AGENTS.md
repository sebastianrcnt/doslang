...
본문은 고치지 않는다. 대신 맨 위에 **상태 헤더**를 둔다. 그것 없이는 읽는
사람이 -- 사람이든 코드 리뷰든 -- 본문의 과거 문장을 지금 살아있는 문제로
오인한다.

```text
Status: open | resolved
Resolved-By: <커밋>        # resolved 일 때만
```

헤더 다음에 무엇이 어떻게 정리됐는지 한 줄. 형식이 고정이라 `Status: open`
하나로 살아있는 audit 을 전부 찾을 수 있다.
