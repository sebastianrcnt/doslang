...
**IR 은 타깃의 폭만 말한다. 호스트의 폭은 IR 에 들어오지 않는다.** 상수, 바이트
오프셋, 크기, 실행 파일에 박히는 줄 번호 — 타깃에게 의미가 있는 값은 `word.h` 의
`FeI32`/`FeU32` 로 나른다. C 의 `long` 은 i386 과 윈도우에서 4바이트, 64비트
유닉스에서 8바이트라, 같은 소스가 컴파일러를 어디서 빌드했느냐에 따라 다른 `.asm`
을 낸다. FNV-1a 기저 `0x811C9DC5` 가 한쪽에서 `-2128831035`, 다른 쪽에서
`2166136261` 로 나온 것이 그 예다 — 비트는 같고 명령어도 같지만 텍스트가 다르고,
그러면 출력을 비교해서 컴파일러를 검사할 수 없다. `tests/determ.py` 가 이것을 지킨다.

`long` 은 컴파일러 밖으로 나가지 않는 것에는 그대로 쓴다 — 루프 카운터, 버퍼 인덱스.

나중에 64비트 타깃을 열면 그것은 여기서의 **의도적인 확장**이지, 빌드가 어디서
돌았는지의 사고가 아니다.
