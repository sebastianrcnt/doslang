...
- `while`은 `bool` 조건만.
- `match`는 **완전성 검사**. enum은 모든 배리언트를, `?T`는 `Some`과 `None`을 모두
  다루거나 `_` 필요.
...
- `comptime if`는 평가되지 않는 분기를 **파싱은 하되 타입 검사/코드 생성하지 않는다**
  (제네릭 인스턴스의 타입별 분기용 — §9. 타깃은 하나이므로 타깃별 분기는 없다, §2).
...
| | |
|---|---|
| `match` | **enum 값과 `?T`에만 쓴다.** 정수·`char`·`bool`에는 쓸 수 없으므로 완전성 검사가 배리언트 목록 하나, 또는 `Some`/`None` 둘로 정해진다. 정수 분기는 `if`/`else if`로 쓴다 |
